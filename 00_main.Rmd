...
## options
nsample = 400 # of samples to create per data set; don't change this, i hard coded in cytodx and save fcm below on which files to use
...
# Calculate summary statistics (don't need to run)
...
# P value vs SpecEnr difference ranking
...
for (result_dir in result_dirs) {
    ...
    try ({
        ...
        write.csv(grt, file=paste0(result_dir,"/summary_table.csv"))
        
        phenolayer = fg@graph$v$phenolayer
        # phenolayer[1] = 1
        
        path_ = paste0(result_dir,"/plots/PvsSpecEnr")
        dir.create(path_, recursive=TRUE, showWarnings=FALSE)
        for (cls in unique(fg@meta$class)) {
            if (cls=="control") next
            p = fg_get_summary(
                fg, type="node", feature="SpecEnr",
                test_name="t_byLayer_diminish", cls="class",
            ...
            data = data.frame(logPvalue=p_, SpecEnrDifference=mse_, col="")
            data$phenotype = ifelse(
                names(p)%in%names(head(sort(p),10)) |
                names(p)%in%names(tail(sort(abs(mse_)),10)), names(p), "")

            gp = ggplot2::ggplot(
                data, aes(x=logPvalue, y=SpecEnrDifference, label=phenotype, color=phenolayer)) +
                ggplot2::geom_point() + 
            ...
            ggplot2::ggsave(paste0(path_,"/",cls,".png"), plot=gp)
...
for (result_dir in result_dirs) {
    ...
    for (cls in unique(fg@meta$class)) {
        try ({
            if (cls=="control") next
            path_ = paste0(result_dir,"/plots/summary/t_byLayer/control_",cls)
            dir.create(path_, recursive=TRUE, showWarnings=FALSE)
            gr = fg_plot(
                fg,
            ...
            p = fg_get_summary(fg, type="node", feature="SpecEnr",
                               test_name="t_byLayer_diminish", cls="class",
                               cls_labels=paste0("control_",cls))$values
            p[mpp_<.001] = 1
            p_ = p<0.01
...
```{r example_cellhierarchy}

result_dirs = list.dirs(paste0(root,"/result"), recursive=FALSE)
result_dir = result_dirs[grepl("pos1$",result_dir)]
fg = get(load(paste0(result_dir,"/fg.Rdata")))
...
start1 = Sys.time()
result_dirs = list.dirs(paste0(root,"/result"), recursive=FALSE)
for (result_dir in result_dirs[-24]) {
    # if (!grepl("genentech",result_dir)) next
    start1 = Sys.time()
    print(result_dir)
    ...
    for (cls in unique(fg@meta$class)) {
        try ({
            if (cls=="control") next
            result_dir_ = paste0(result_dir,"/plots/ratios/control_",cls)
            dir.create(result_dir_, recursive=TRUE, showWarnings=FALSE)
            p = fg_get_summary(fg, type="node", feature="SpecEnr",
                               test_name="t_byLayer", cls="class",
                               cls_labels=paste0("control_",cls)) $values
            cpops = names(head(sort(p),30))
            for (cpopi in seq_len(length(cpops))) {
                ...
                ci = fg@meta[["class"]]=="control"
                a1 = fg@feat$node$expect_prop[ci,cpop]
                a2 = fg@feat$node$prop[ci,cpop]
                a = fg@feat$node$SpecEnr[ci,cpop]
                ei = fg@meta[["class"]]==cls
                b1 = fg@feat$node$expect_prop[ei,cpop]
                b2 = fg@feat$node$prop[ei,cpop]
                b = fg@feat$node$SpecEnr[ei,cpop]
                ...
                # plot
                try ({
                    gp1 = ggplot() + ggtitle(paste0("population ",cpop,"\nred=control, blue=experiment","\nexpected/actual proportion", "\n SpecEnr t test p value (by layer adjustment): ", p[cpop],"\nexpected proportion means: ", round(mean(a1),3),"/",round(mean(a2),3),", ",round(mean(b1),3),"/",round(mean(b2),3))) +
                        geom_density(aes(x=x), colour="red",data=data.frame(x=a)) +
                        geom_density(aes(x=x), colour="blue",data=data.frame(x=b))
                    
                    ggsave(paste0(result_dir_,"/",str_pad(cpopi,width=3,pad="0"),"_",cpop,".png"), plot=gp, scale=1, width=5, height=5, units="in", dpi=500, limitsize=TRUE)
                ...
                # try({
                #     
                #     gp2 = ggplot() + ggtitle(paste0("population ",cpop,"\nred=control, blue=experiment","\nexpected/actual proportion", "\n t test p value: ", p[cpop],"\nmeans: ", round(mean(log(a1)),3),"/",round(mean(log(a2)),3),", ",round(mean(log(b1)),3),"/",round(mean(log(b2)),3))) +
                #         geom_density(aes(x=x), colour="red",data=data.frame(x=a)) +
                #         geom_density(aes(x=x), colour="blue",data=data.frame(x=b))
                #     
                #     ggsave(paste0(result_dir_,"/",signif(p[cpop],3),"_",cpop,"_log.png"), plot=gp, scale=1, width=5, height=5, units="in", dpi=500, limitsize=TRUE)
                # })
