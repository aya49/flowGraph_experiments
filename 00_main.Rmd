    ...
    for (cls in unique(fg@meta$class)) {
        try ({
            ...
            gr = fg_plot(
                ...
                label1="expect_prop", label2="prop", # node only
                p_thres=0.01, show_bgedges=TRUE)
                #path=paste0(path_,"/SpecEnr.png"))
...
start1 = Sys.time()
result_dirs = list.dirs(paste0(root,"/result"), recursive=FALSE)
for (result_dir in result_dirs[5:55]) {
    # if (!grepl("genentech",result_dir)) next
    start1 = Sys.time()
    print(result_dir)
    ...
    for (cls in unique(fg@meta$class)) {
        try ({
            ...
            p = fg_get_summary(fg, type="node", feature="SpecEnr",
                               test_name="t_byLayer", cls="class",
                               cls_labels=paste0("control_",cls)) $values
            for (cpop in names(head(sort(p),30))) {
                if (cpop=="") next
                if (p[cpop]==1) next
                # cpop = "A+B+C+"
                ...
                try ({
                    ...
                    ggsave(paste0(result_dir_,"/",signif(p[cpop],3),"_",cpop,".png"), plot=gp, scale=1, width=5, height=5, units="in", dpi=500, limitsize=TRUE)
