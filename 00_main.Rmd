...
fg = flowGraph(ft_dirs, meta=meta_file0, no_cores=no_cores, label1="control",
               norm_path=paste0(result_dir0,"/count_norm"),calculate_summary=FALSE)
# fg <- fg_feat_mean_class(fg, class="subject", no_cores=1, 
#                          node_features=c("prop","SpecEnr"), edge_features="NONE")
...
#paste0("ctrl",c(0:9)),
#paste0("pos",c(1:30))
for (ds in c(paste0("pos",c(26,17,12,14,16,18,19)))){#,paste0("ctrl",c(0:9)))) {
    start2 = Sys.time()
...
for (result_dir in result_dirs) { try({
    ...
    fg = get(load(paste0(result_dir,"/fg.Rdata")))
    if (grepl("pregnancy|bodenmiller", result_dir)) {
        fg_save_plots(fg, paste0(result_dir,"/fg/plots"), box_no=15, paired=TRUE)
    } else {
        fg_save_plots(fg, paste0(result_dir,"/fg/plots"), box_no=15)
    }
    ...
    for (se_ind in se_inds) { try({
        ...
        for (absolute in c(TRUE, FALSE)) {
            for (logged in c(TRUE, FALSE)) {
                ...
                if (logged) {
                    dfa$x <- -log(pp1$values[ito]) + log(pp1$values[ifrom])
                    dfa$y <- -log(pp2$values[ito]) + log(pp2$values[ifrom])
                } else {
                ...
                for (layers in 7:1) {
                    for (layers_ in layers:1) {
                        f_inds <- dfa$phenolayer >= layers_ & dfa$phenolayer <= layers
                        
                        sigv <- c("all","insig","sig")
                        paths_ <-  apply(expand.grid(c(0,1,2), c(0,1,2), c(0,1,2), c(0,1,2)), 1, function(sg) {
                            sg <- unlist(sg)
                            paste0(
                                plot_path_,"/SpecEnrVSraw", 
                                ifelse(absolute,"_abs",""), 
                                ifelse(logged,"_ln",""), 
                                "_fromLayer", layers,"-to-",layers_,
                                "_SpecEnr-",sigv[sg[1]+1], "FROM-",sigv[sg[2]+1], 
                                "TO_raw-", sigv[sg[3]+1], "FROM-",sigv[sg[4]+1], 
                                "TO.png")
                        })
                        inds_ <- apply(expand.grid(c(0,1,2), c(0,1,2), c(0,1,2), c(0,1,2)), 1, function(sg) {
                        ...
                        if (length(inds_good)>0)
                            a <- sapply(inds_good, function(i) {
                                qp <- ggplot2::ggplot(dfa[inds_[,i],], ggplot2::aes(
