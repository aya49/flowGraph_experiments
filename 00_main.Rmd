    ...
    for (cls in unique(fg@meta$class)) {
        try ({
            ...
            gr = fg_plot(
                ...
                label1="expect_prop", label2="prop", # node only
                p_thres=0, show_bgedges=TRUE)
                #path=paste0(path_,"/SpecEnr.png"))
            
            p = fg@feat_summary$node[[5]]$values
            p_ = p==0
            ...
            for (mrks in unique(levi_0_mrks)) {
                ...
                if (max(diffsi)==0)  # BETTTER IF THERES A THRESHOLD
            ...
            gr$v$size = sediff#abs(exp_m-exp_c)
            gr$v$label_ind = gr$v$v_ind = sediff > sediffthres# & smdiff < .01
            gp = gggraph(gr, main="Feature: SpecEnr\nClass: class - control_exp\nsummary statistic: t_byLayer (threshold: 0)\nNode size & colour: non-repeating SpecEnr (exp-0) of nodes with same markers WITH NO DUPLICATES.\nLabel(s): SpecEnr (exp-0), node, expect_prop (prop)")
            ...
            gp = gggraph(gr, main="Feature: SpecEnr\nClass: class - control_exp\nsummary statistic: t_byLayer (threshold: 0)\nNode size & colour: non-repeating prop (exp-0) of nodes with same markers.\nLabel(s): SpecEnr (exp-0), node, expect_prop (prop)")
            ...
            gp = gggraph(gr, main="Feature: SpecEnr\nClass: class - control_exp\nsummary statistic: t_byLayer (threshold: 0)\nNode size & colour: non-repeating SpecEnr (exp-0) or prop of nodes with same markers.\nLabel(s): SpecEnr (exp-0), node, expect_prop (prop)")
            ...
            for (lev in sort(unique(gr$v$phenolayer), decreasing=TRUE)[-1]) {
                levi_0_ = which(p_ & spdiff > sediffthres & 
                                    lyrsl[[as.character(lev+1)]] & ppind)
                levi_0 = which(p_ & spdiff > sediffthres & 
                ...
                for (chld in names(levi_0_)) {
                    ...
                    if (length(prnt_1)==1) {
                        ...
                        if (max(diffsp)==0) # BETTTER IF THERES A THRESHOLD
            ...
            gp = gggraph(gr, main="Feature: SpecEnr\nClass: class - control_exp\nsummary statistic: t_byLayer (threshold: 0)\nNode size & colour: non-repeating SpecEnr (exp-0) of nodes with same markers and of nodes who are parents of a single node.\nLabel(s): SpecEnr (exp-0), node, expect_prop (prop)")
...
result_dir_ = paste0(result_dir,"/plots/ratios")
for (cpop in fg@graph$v$phenotype[-1]) {
    ...
    try ({
        ...
        ggsave(paste0(result_dir_,"/",cpop,".png"), plot=gp, scale=1, width=5, height=5, units="in", dpi=500, limitsize=TRUE)
