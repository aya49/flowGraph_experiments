...
libr(c("flowCore", "flowType", "RchyOptimyx",
       "doParallel", "foreach",
       "stringr", "furrr", "purrr",
       "plotly", "htmlwidgets", "ggpubr", "effsize"))
...
fg = flowGraph(ftl, no_cores=no_cores, meta=meta_file, class="class",
               ...
               test_custom=function(x,y)
                   tryCatch(stats::wilcox.test(x,y, paired=TRUE)$p.value,
                            error=function(e) 1),
...
inds <- grepl(paste0(meta_id,collapse="|"), names(mcl))
...
fg <- flowGraph(mc, cls=meta_class, no_cores=no_cores, calculate_summary=FALSE)
...
# save
dir.create(paste0(root,"/result/epichipc_bcell"))
save(fg, file=paste0(root,"/result/epichipc_bcell/fg.Rdata"))
# save_fg(fg, paste0(root,"/result/epichipc_bcell/fg"))
...
fg = flowGraph(ft_dirs, meta=meta_file0, no_cores=no_cores, label1="control",
               norm_path=paste0(result_dir0,"/count_norm"),
               test_custom=function(x,y)
                   tryCatch(stats::wilcox.test(x,y, paired=TRUE)$p.value,
                            error=function(e) 1),
               path=paste0(result_dir0,"/fg"))
...
p75 = quantile(cvd, .75)
p25 = quantile(cvd, .25)
thress0 = map(markers, function(x) p50); names(thress0) = markers
...
#paste0("ctrl",c(0:9)),
#paste0("pos",c(1:30))
for (ds in c(paste0("pos",c(1:39)))){#,paste0("ctrl",c(0:9)))) {
    start2 = Sys.time()
    ...
    ftl = future_map(loop_ind, function(ii) {
        future_map(ii, function(i) {
            ...
            if (i>(nsample*nctrl) & grepl("pos",ds)) {
                ...
                # }
                else if (ds=="pos18" | ds=="pos38") { #1.5x A+B+C+, C+D+, like above
                    tm = sum(triple)/2
                    tm2 = sum(double)/2
                    f@exprs = rbind(f@exprs,f@exprs[sample(which(ap & bp & cp),tm),])
                ...
                #               sample(which(!ap & bp),tm/3)),c(1,2)] = p75
                # }
                else if (ds=="pos19" | ds=="pos39") { #1.5x A+, A+B+C+
                    tm = sum(triple)/2
                    f@exprs = rbind(f@exprs,f@exprs[sample(which(ap & bp & cp),tm),])
...
for (result_dir in result_dirs) {try({
    ...
    if (!grepl("pregnancy|bodenmiller", result_dir)) {
        ...
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, 
                        test_name="wilcox_diminish", adjust_custom="none",
                        node_features=c("prop_MEAN_subject","SpecEnr_MEAN_subject"), edge_features="NONE")
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, test_custom="t",
                        test_name="t_diminish", adjust_custom="none",
                        node_features=c("prop_MEAN_subject","SpecEnr_MEAN_subject"), edge_features="NONE")
    ...
    feat_orig <- ifelse("cells_ul_blood"%in%names(fg@feat$node), 
                        "cells_ul_blood", "prop")
    
    qt <- function(x,y)
        min( sum(x<.1 & x>-.1)/length(x), sum(y<.1 & y>-.1)/length(y))
    
    for (feat in c(feat_orig, paste0("SpecEnr_",feat_orig))) {
        if (feat=="SpecEnr_prop") feat <- "SpecEnr"
        label2 <- unlist(tail(fg@summary_desc$node$label2,1))
        if (label2=="mix") label2 <- "aml"
        
        fg <- fg_summary(fg, no_cores=no_cores, test_name="test0",
                         test_custom=qt, class="class", 
                         label1="control", label2=label2,
                         adjust_custom="none", diminish=FALSE,
                         node_features=feat, edge_features="NONE",
                         overwrite=FALSE)
        
        test_name <- paste0(ifelse("wilt"%in%fg@summary_desc$node$test_name, 
                                   "wilt", "wilcox"), "_byLayer_diminish")
        summary_meta0 <- c(feature=feat, test_name=test_name, 
                           class=unlist(tail(fg@summary_desc$node$class,1)),
                           label1=unlist(tail(fg@summary_desc$node$label1,1)),
                           label2=label2)
        index0 <- flowGraph::fg_get_summary_index(fg, type="node", summary_meta=summary_meta0)
        pv <- fg_get_summary(fg, index=index0)$values
        
        summary_meta1 <- summary_meta0
        summary_meta1[2] <- "test0"
        index1 <- flowGraph::fg_get_summary_index(fg, summary_meta=summary_meta1)
        pv[fg_get_summary(fg, index=index1)$values<.5] <- 1
        
        summary_meta2 <- summary_meta0
        summary_meta2[2] <- paste0(summary_meta2[2],"_adjust0")
        fg <- fg_add_summary(
            fg, type="node", summary_meta=unlist(list(
                feat=summary_meta2[1], test_name=summary_meta2[2], 
                class=summary_meta2[3], label1=summary_meta2[4],
                label2=summary_meta2[5])),
            overwrite=TRUE, p=list(values=pv))
    }
    save(fg, file=paste0(result_dir,"/fg.Rdata"))
    rm(fg); gc()
})}

# calculate efect sizes
for (result_dir in result_dirs) {try({
    start <- Sys.time()
    cat(result_dir,"\n")
    fg = get(load(paste0(result_dir,"/fg.Rdata")))
...
for (result_dir in result_dirs) { 
    ...
    if (!grepl("pregnancy|bodenmiller", result_dir)) {
        ...
        a = fg_plot_dist(fg, node_features=c("count","prop", "SpecEnr"),
                         edge_features="NONE",
                         class="class",
                         dist_method="euclidean", get_means=NULL,
                         plot_path=paste0(result_dir,"/plots/distance.png"))
        
    } else {
        a = fg_plot_dist(fg, node_features=c("count","prop", "SpecEnr"),
                         edge_features="NONE",
                         class="class",
                         dist_method="euclidean", get_means="subject",
                         plot_path=paste0(result_dir,"/plots/distance.png")) 
    }
}
...
for (result_dir in result_dirs) { try({
    ...
    fg = get(load(paste0(result_dir,"/fg.Rdata")))
    if (!grepl("pregnancy|bodenmiller", result_dir)) {
        fg_save_plots(fg, paste0(result_dir,"/fg/plots"))
    } else {
        fg_save_plots(fg, paste0(result_dir,"/fg/plots"), paired=TRUE)
    }
    ...
    for (index in seq_len(length(fg@summary[[type]]))) {
        ...
        try ({
            ...
            gr <- fg_plot(
                fg, type="node", p_thres=.01, index=index,
                show_bgedges=ifelse(grepl("pos",result_dir),TRUE, FALSE),
                label_max=15,
                node_labels="NONE")
            if (is.null(gr)) next
            ...
            gp <- plot_gr(gr, bgedges=ifelse(grepl("pos",result_dir),TRUE, FALSE))
