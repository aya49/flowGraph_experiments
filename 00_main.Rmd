...
mcl <- unlist(purrr::map(ft_paths, function(x) {
    a <- read.csv(gsub("//","/",x))
    ua <- unique(a$samples_name)
    b <- purrr::map(ua, function(y)
        a$cell_counts[a$sample_name==y])
    names(b) <- ua
    b
}),
retursive=FALSE)
...
phenocode <- stringr::str_pad(
    read.csv(gsub("//","/",ft_paths[1]))$Pheno_codes,
    width=nchar(as.character(max(a$Pheno_codes))), side="left", pad="0")
colnames(mc) <- phenotype <- read.csv(gsub("//","/",ft_paths[1]))$phenotype_names
...
for (result_dir in result_dirs) {
    ...
    try ({
        ...
        for (index in seq_len(length(fg@summary$node))) {
            ...
            fg_plot_rchy(fg, index=index, phen_start_n=3, pathCount=5)
            dev.off()
        }
    })
}
