...
mc <- do.call(rbind, mcl)
rownames(mc) <- names(mcl)
...
fg <- fg_add_feature(
    ...
    m=meta_cb*fg@feat$node$expect_prop)
...
fg <- fg_summary(fg, no_cores=no_cores, test_name="wilt",, adjust_custom="none",
                 ...
                 node_features=c("prop","SpecEnr", "cells_ul_blood",
                                 "SpecEnr_cells_ul_blood"),
                 overwrite=FALSE)
fg <- fg_summary(fg, no_cores=no_cores, test_name="wilt_diminish",, adjust_custom="none",
...
#  The total size of the 5 globals that need to be exported for the future expression (â€?.f()â€?) is 746.88 MiB. This exceeds the maximum allowed size of 500.00 MiB (option 'future.globals.maxSize'). The three largest globals are â€˜epâ€? (579.19 MiB of class â€˜numericâ€?), â€˜mpâ€? (102.64 MiB of class â€˜numericâ€?) and â€˜pparenâ€? (55.13 MiB of class â€˜listâ€?).
...
# Distance matrices comparison between features
```{r}
...
for (result_dir in result_dirs) {
    ...
    fg = get(load(paste0(result_dir,"/fg.Rdata")))
    if (!grepl("pregnancy|bodenmiller", result_dir)) {
        save_fg_plots(fg, paste0(result_dir,"/plots"))
    } else {
        save_fg_plots(fg, paste0(result_dir,"/plots"), paired=TRUE)
    }
}
```
        ...
        for (index in seq_len(length(fg@summary[[type]]))) {
            ...
            try ({
                ...
                gr <- fg_plot(fg, type="node", p_thres=.01, index=index,
                              show_bgedges=FALSE, path=path, label_max=15,
                              node_labels="NONE")
                if (is.null(gr)) next
