...
#paste0("ctrl",c(0:9)),
#paste0("pos",c(1:30))
for (ds in c(paste0("pos",c(1:36)))){#,paste0("ctrl",c(0:9)))) {
    start2 = Sys.time()
        ...
        future_map(ii, function(i) {
            ...
            if (i>(nsample*nctrl) & grepl("pos",ds)) {
                ...
                if (ds=="pos1") { # A+ > .75; A- > .25
                    ...
                    f@exprs = rbind(
                        f@exprs, f@exprs[sample(which(ap),sum(ap)/2),],
                        f@exprs[sample(which(cp & dp & bp),sum(triple)/2),])
                }
                    ...
                    f@exprs = rbind(
                        f@exprs, f@exprs[sample(which(ap),sum(ap)/2),],
                        f@exprs[sample(which(bp),sum(bp)/2),])
                }
                    ...
                    f@exprs = rbind(
                        ...
                        f@exprs[sample(which(bp & cp & dp),tm),])
                }
                else if (ds=="pos17") { #1.5x D+, A+B+C+
                    f@exprs = rbind(
                        f@exprs, f@exprs[sample(which(dp), sum(dp)/2),],
                        f@exprs[sample(which(ap & bp & cp), sum(triple)/2),])
                }
                ...
                # }
                    ...
                    f@exprs = rbind(
                        f@exprs, f@exprs[sample(which(cp & dp), sum(double)/2),],
                        f@exprs[sample(which(ap & bp & cp), sum(triple)/2),])
                }
                ...
                # }
                    ...
                    f@exprs = rbind(
                        f@exprs,
                        f@exprs[sample(which(ap),sum(ap)/2),],
                        f@exprs[sample(which(ap & bp & cp),sum(triple)/2),])
                }
                    ...
                    f@exprs = rbind(
                        f@exprs, f@exprs[sample(which(dp),sum(dp)/2),],
                        f@exprs[sample(which(ap & bp),sum(double)/2),])
                    # tm = sum(double)/2
...
for (result_dir in result_dirs) {try({
    ...
    if (grepl("pregnancy|bodenmiller", result_dir)) {
        ...
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, 
                        test_name="wilcox_diminish", adjust_custom="none",
                        test_custom=qw,
                        node_features=c("prop","SpecEnr"), edge_features="NONE")
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, 
                        test_name="wilcox_BH_diminish", adjust_custom="BH",
                        test_custom=qw,
                        node_features=c("prop","SpecEnr"), edge_features="NONE")
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, test_name="wilcox_byLayer_diminish",
                        test_custom=qw,
                        diminish=TRUE,
                        node_features=c("prop","SpecEnr"), edge_features="NONE")
        ...
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, 
                        test_name="t_diminish", adjust_custom="none",
                        test_custom=qt,
        ...
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        ...
                        node_features=c("prop","SpecEnr"), edge_features="SpecEnr")
        fg = fg_summary(fg, no_cores=no_cores, class="class", label1="control",
                        overwrite=FALSE, test_name="t_byLayer_diminish",
                        test_custom=qt,
                        diminish=TRUE,
                        node_features=c("prop","SpecEnr"), edge_features="SpecEnr")
        ...
        fg <- fg_summary(fg, no_cores=no_cores, test_name="wilt_byLayer_diminish",
                         ...
                         overwrite=FALSE)
        fg <- fg_summary(fg, no_cores=no_cores, test_name="wilt_BH_diminish",
                         test_custom=test_custom, adjust_custom="BH",
                         label1="control",
                         node_features=c("cells_ul_blood",
                                         "SpecEnr_cells_ul_blood"),
                         edge_features="SpecEnr",
        ...
        fg <- fg_summary(fg, no_cores=no_cores, test_name="wilt",
                         test_custom=test_custom, adjust_custom="none",
                         diminish=FALSE, label1="control",
                         node_features=c("cells_ul_blood",
                                         "SpecEnr_cells_ul_blood"),
                         edge_features="SpecEnr",
                         overwrite=FALSE)
...
for (result_dir in result_dirs) { try({
    ...
    fg = get(load(paste0(result_dir,"/fg.Rdata")))
    if (grepl("pregnancy|bodenmiller", result_dir)) {
        fg_save_plots(fg, paste0(result_dir,"/fg/plots"), box_no=0, paired=TRUE)
    } else {
    ...
    for (index in seq_len(length(fg@summary[[type]]))) {
        ...
        fg_plot_qq(fg, type="node", index=index, logged=TRUE,
           path=paste0(plot_path_,"/qq_logged.png"))
        fg_plot_qq(fg, type="node", index=index, logged=F,
           path=paste0(plot_path_,"/qq.png"))
        fg_plot_qq(fg, type="node", index=index, logged=F,
           path=paste0(plot_path_,"/qq_filtered.png"), filter_adjust0 = .4)
        fg_plot_qq(fg, type="node", index=index, logged=T,
           path=paste0(plot_path_,"/qq_logged_filtered.png"), filter_adjust0 = .4)

        fg_plot_pVSdiff(fg, index=index, interactive=F,
                path=paste0(plot_path_,"/pVSdifference.png"))
        fg_plot_pVSdiff(fg, index=index, interactive=F,
                path=paste0(plot_path_,"/pVSdifference_filtered.png"), filter_adjust0 = .4)
        
        # cell hierarchy plots without number labels
        try ({
            ## cell hierarchy plot
            p <- fg_get_summary(fg, type=type, index=index)$values
            nlabels <- c("prop", "expect_prop")
            if (grepl("epichipc",result_dir)) 
                nlabels <- c("cells_ul_blood", "expect_cells_ul_blood")
            gr <- fg_plot(
                fg, type=type, p_thres=.01, index=index,
                show_bgedges=ifelse(grepl("pos",result_dir), TRUE, FALSE),
                path=paste0(plot_path_,"/cell_hierarchy"), interactive=TRUE,
                node_labels=nlabels)
            if (is.null(gr)) next
...
for (result_dir in result_dirs) {
    ...
    pp1 <- fg_get_summary(fg, index=se_inds[1])
    ...
    mp <- fg_get_feature(fg, type="node", feature=compare_feats[2])
    mep <- fg_get_feature(fg, type="node", feature=paste0("expect_",compare_feats[2]))
    
    pp3v1_ <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::t.test(mp[pp1$id1,ci], mep[pp1$id1,ci], paired=TRUE)$p.value,
                 error=function(e) 1) )
    pp3v2_ <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::t.test(mp[pp1$id2,ci], mep[pp1$id2,ci], paired=TRUE)$p.value,
                 error=function(e) 1) )
    pp3v1 <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::t.test(mp[pp1$id1,ci], mep[pp1$id1,ci])$p.value,
                 error=function(e) 1) )
    pp3v2 <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::t.test(mp[pp1$id2,ci], mep[pp1$id2,ci])$p.value,
                 error=function(e) 1) )
    
    pw3v1_ <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::wilcox.test(mp[pp1$id1,ci], mep[pp1$id1,ci], paired=TRUE)$p.value,
                 error=function(e) 1) )
    pw3v2_ <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::wilcox.test(mp[pp1$id2,ci], mep[pp1$id2,ci], paired=TRUE)$p.value,
                 error=function(e) 1) )
    pw3v1 <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::wilcox.test(mp[pp1$id1,ci], mep[pp1$id1,ci])$p.value,
                 error=function(e) 1) )
    pw3v2 <- sapply(seq_len(ncol(mp)), function(ci) 
        tryCatch(stats::wilcox.test(mp[pp1$id2,ci], mep[pp1$id2,ci])$p.value,
                 error=function(e) 1) )
    
    pp3v1_[is.na(pp3v1_)] <- pp3v2_[is.na(pp3v2_)] <- pp3v1[is.na(pp3v1)] <- pp3v2[is.na(pp3v2)] <- pw3v1_[is.na(pw3v1_)] <- pw3v2_[is.na(pw3v2_)] <- pw3v1[is.na(pw3v1)] <- pw3v2[is.na(pw3v2)] <- 1
    
    pp3c1 <- sapply(seq_len(ncol(mp)), function(ci) 
        effsize::cohen.d(mp[pp1$id1,ci], mep[pp1$id1,ci])$estimate )
    pp3c2 <- sapply(seq_len(ncol(mp)), function(ci) 
        effsize::cohen.d(mp[pp1$id2,ci], mep[pp1$id2,ci])$estimate )
    pp3c1[is.na(pp3c1)] <- pp3c2[is.na(pp3c2)] <- 0
    # |d|<0.2 "negligible", |d|<0.5 "small", |d|<0.8 "medium", otherwise "large"
    names(pp3c1) <- names(pp3c2) <- names(pp3v1) <- names(pp3v2) <- names(pp3v1_) <- names(pp3v2_) <- fg@graph$v$phenotype
    
    df_ps <- data.frame(
        phenotype=fg@graph$v$phenotype,
        tpv1_paired=pp3v1_, tpv2_paired=pp3v2_,
        tpv1=pp3v1, tpv2=pp3v2,
        cd1=pp3c1, cd2=pp3c2
    )
    fg@etc$actualVSexpect$node[[compare_feats[1]]] <- df_ps
    
    # edge version of above
    mp <- fg_get_feature(fg, type="edge", feature="prop")
    mep <- fg_get_feature(fg, type="edge", feature="expect_prop")
    ...
    pp3c1[is.na(pp3c1)] <- pp3c2[is.na(pp3c2)] <- 0
    # |d|<0.2 "negligible", |d|<0.5 "small", |d|<0.8 "medium", otherwise "large"
    names(pp3c1) <- names(pp3c2) <- names(pp3v1) <- names(pp3v2) <- names(pp3v1_) <- names(pp3v2_) <- colnames(mp)
    ...
    fg@etc$actualVSexpect$edge$SpecEnr <- df_ps_
...
for (result_dir in result_dirs) {
    ...
    # whether specenr should be significant based on how different actual
    # and expected raw values are; sigcands are those that are significantly different
    df_ps <- fg@etc$actualVSexpect$node[[compare_feats[1]]]
    ...
    for (se_ind in se_inds) { try({
        summary_meta <- fg@summary_desc[[type]][se_ind,]
        plot_path_ <- paste0(result_dir,"/fg/plots/", type, "/",
                             paste0(unlist(summary_meta), collapse="_"))
        dir.create(plot_path_, recursive=TRUE, showWarnings=FALSE)
        ...
        pp1 <- fg_get_summary(fg, index=se_ind)
        ...
        pp2 <- fg_get_summary(fg, summary_meta=unlist(append(compare_feats[2], fg@summary_desc$node[se_ind,-1]))) # error if not found
                        ...
                        if (interactive) {
                            qp <- qp +
                                ggiraph::geom_point_interactive(
                                    ggplot2::aes(tooltip=label_long,
                                                 alpha=.1,stroke=1))
                            htmlwidgets::saveWidget(
                                ggiraph::girafe(ggobj=qp),
                            ...
                            ggplot2::ggsave(
                                paths_[i], plot=qp, scale=1, width=5, height=5,
                                units="in", dpi=500, limitsize=TRUE)
                        }
                        
                        # edge comparison
                        if (sum(sig_combos[i,c(1,2)])==0) {
                            qp <- ggplot2::ggplot(df[inds_[,i],], ggplot2::aes(
