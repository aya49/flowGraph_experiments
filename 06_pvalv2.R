...
for (result_dir in result_dirs) {
  ...
  sum_p_dir = paste0(sum_dir,"/pval"); dir.create(sum_p_dir, recursive=T, showWarnings=F)
  sum_m_dir = paste0(sum_dir,"/mean"); dir.create(sum_m_dir, showWarnings=F)
  ...
  table = rbind(table, ldply(feat_types, function(feat_type) {
    start2 = Sys.time()
    cat("\n", feat_type, " ",sep="")
    ...
    for (uc in unique(sm$class[!controli])) {
      ...
      pvs = llply(loopInd(1:ncol(m),no_cores), function(jj) {
        llply(jj, function(j) {
          ...
          t = wilcox = list()
          t$test = t$train = t$all = #t$train2 =
            wilcox$test = wilcox$train = wilcox$all = #wilcox$train2=
            1
          ...
          # }
          if (!all(emte==emte[1])) {
            t$test = t.test(emte,cmte)$p.value
            wilcox$test = wilcox.test(emte,cmte)$p.value
          }
          ...
          # }
          if (!all(emtr==emtr[1])) {
            t$train = t.test(emtr,cmtr)$p.value
            wilcox$train = wilcox.test(emtr,cmtr)$p.value
          }
      ...
      for (ptype in names(pvs[[1]])) {
        for (adj in adjust) {
          ...
          save(pvs,file=pname)
          tbl = rbind(tbl, data.frame(data=data,feat=feat_type,class=uc,test=ptype,test_adj=adj,pathp=pname,pathm=mname))
    ...
    # save(foldsip, file=paste0(sum_p_dir,"/",feat_type,".Rdata"))
    # save(foldres, file=paste0(sum_p_dir,"/",feat_type,"_table.Rdata"))
    save(fmean, file=mname)
    time_output(start2)
    # foldsip = get(load(paste0(sum_p_dir,"/",feat_type,".Rdata")))
    # foldres = get(load(paste0(sum_p_dir,"/",feat_type,"_table.Rdata")))
...
save(table, file=paste0(root,"/pvals.Rdata"))
time_output(start)
