...
matrix_count = c("CountAdj")
matrix_type = c("PvalTRIM_CountAdj", "LogFoldTRIM_CountAdj", "Parent_effortTRIM", "Parent_contribTRIM")
...
for (ci in 1:length(paste0(panelL,centreL))) {
  ...
  for (mcp in c(weight_matrix,matrix_type)) {
    ...
    if (!file.exists(paste0(matrix_dir[ci], mcp,".Rdata"))) {cat("doesn't exist"); break}
  ...
  for (countThres in cellCountThres[ci]) {
    ...
    for (k in k0) {
        ...
        if (is.null(dim(m))) {
          ...
          for (i in 2:length(gt)) {
            dm[1:(i-1),i] = dm[i,1:(i-1)] = foreach(j = 1:(i-1), .combine="c") %dopar% {
              ...
              diff = abs(m[j,]-m[i,]) #manhattan
              weight = pmax(mw[i,],mw[j,])
              dist_m = sum(weight[diff!=0]*diff[diff!=0])
              return(dist_m)
      ...
      save(d,file=paste0(dname,"_original.Rdata"))
      ...
      for (s in 1:3) {
        ...
        d0 = foreach(i=1:length(d)) %dopar% { return(matrix_weights[i]*dn[[i]]) }
        d1 = Reduce("+",d0)
