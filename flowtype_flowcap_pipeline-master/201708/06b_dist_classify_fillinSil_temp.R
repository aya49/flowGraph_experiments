...
a = foreach(i=1:length(distmfile)) %dopar% {
  tryCatch({
    ...
    mmorig = NULL
    if (any(cltypesorigm%in%cltypes)) {
      mresult = Loadintermatrices(paste0(matrix_dir,distMeta$type[i],".Rdata"),verbose=F)
      phenolevel = sapply(colnames(mresult$mml[[1]]), function(x){return(length(strsplit(x,"[+-]")[[1]])) } )
      mmlresult = trimMatrices(mresult$mml,m0,mresult$pt,mresult$gt,phenolevel,NULL,F,distMeta$count[i],distMeta$layer[i])
      mmorig = mmlresult$mml[[1]]
      pm = mmlresult$pm
      if ((is.null(dim(mmorig)) & distMeta$norm[i]=="none" & all(!distMeta[i,c("rw","sim","weighted")]) ) | (!is.null(dim(mmorig)) & distMeta$norm[i]=="cellpop" & all(!distMeta[i,c("rw","sim","weighted")]))) {
        if (is.null(dim(mmorig))) mmorig = Reduce("cbind",mmorig)
      } else { mmorig = NULL }
    }
    if (is.null(rownames(d0))) {
      rownames(d0) = colnames(d0) = rownames()
    }
    ...
    fm = list()
    if (!(!file.exists(paste0(dist_clusterf_dir,"/",distmfilenames[i])) | overwritef)) fm = get(load(paste0(dist_clusterf_dir,"/",distmfilenames[i])))
    
    cm0 = list()
    if (!(!file.exists(paste0(dist_clustercl_dir,"/",distmfilenames[i])) | overwritecl)) cm0 = get(load(paste0(dist_clustercl_dir,"/",distmfilenames[i])))
    
    #for each interested column
    for (col in 1:length(interested)) { 
      colnam = paste0("interested-",interested[col],"_splitby-",splitby[col])
      if (!colnam%in%names(fm) | overwritef) fm[[colnam]] = list()
      if (!colnam%in%names(cm0) | overwritecl) cm0[[colnam]] = list()
      ...
      for (dind in 1:length(d)) {
        ...
        laname = sort(unique(sm[[dind]][,interested[col]]))
        ...
        if (!dindname%in%names(cm0[[colnam]]) | overwritecl) cm0[[colnam]][[dindname]] = list()
        ...
        if (docl) {
          ...
          for (cltype in cltypes) {
            ...
            if (!cltype%in%names(cm0[[colnam]][[dindname]]) | overwritecl) {
              ...
              if (cltype=="spec") { 
                ...
                if (savekern) { 
                  ...
                  kern_dir = paste0(dist_kern_dir,"/",gsub(".Rdata","",dis_dir_temp))
              ...
              if (is.null(clt)) {
                ...
                } else { cm0[[colnam]][[dindname]] = cm0[[colnam]][[dindname]][-length(cm0[[colnam]][[dindname]])] } 
                next 
              }
              clt0 = clt
              leavcol = !apply(clt,2,function(x) return(length(unique(x))>maxcl))
              if (sum(leavcol)<ncol(clt)) {
                if (sum(leavcol)==1) {
                  clt = clt
                  clt = matrix(clt)
                  rownames(clt) = rownames(clt0)
                  colnames(clt) = colnames(clt0)[!apply(clt,2,function(x) return(length(unique(x))>maxcl))]
                } else if (sum(leavcol)==0) { next 
                } else { clt = clt[,leavcol] }
              }
              
              cltndupind = !duplicated(t(clt))
              if (sum(cltndupind)<ncol(clt)) {
                cname = colnames(clt)[cltndupind]
                clt = clt[,cltndupind]
                if (sum(cltndupind)==1) {
              ...
              clt1 = clt
              clt1[1:nrow(clt1),1:ncol(clt1)] = NA
              for (j in 1:ncol(clt)) {
                cl = clt[,j]
                
                # cl1 for traditional f measure, should have same clusters as label; length(cl1)<=length(la)
                if (length(cl)==length(la1)) { #if supervised classification
                  la = la1 #for all metrics
                  cl1 = cl
                } else { #clustering
        ...
        if (dof | length(cm0[[colnam]][[dindname]])>0) {
          cat(" scoring (",sep="")
          if (!dindname%in%names(fm[[colnam]]) | overwritef) fm[[colnam]][[dindname]] = list()
          
          fm[[colnam]][[dindname]]$sil = silhouette(la0,as.dist(d[[dindname]]))[,3]
          
          for (cltype in names(cm0[[colnam]][[dindname]])[names(cm0[[colnam]][[dindname]])!="sil"]) {
            start2 = Sys.time()
            cat(" ",cltype,sep="")
            clt = NULL
            par = clpar[cltype]
            ...
            if (!cltype%in%fm[[colnam]][[dindname]] | overwritef)  fm[[colnam]][[dindname]][[cltype]] = list()
            ...
            for (j in 1:ncol(clt)) {
              ...
              cl1silmed = clsilmed = NA
              if (!cltype%in%cltypesclass) {
                if (length(unique(cl1))>1) cl1silmed = median(silhouette(cl1,d[[dind]]))
                if (length(unique(cl))>1) clsilmed = median(silhouette(cl,d[[dind]]))
              }
              ...
              eval = c(silmed=median(fm[[colnam]][[dindname]]$sil),
                       cl1silmed=cl1silmed,cl1eval,
                       cl1.oneClass=cl1.oneClass,cl1Equalcl=cl1Equalcl,cl.oneClass=cl.oneClass,
                       clsilmed=clsilmed,cleval)
