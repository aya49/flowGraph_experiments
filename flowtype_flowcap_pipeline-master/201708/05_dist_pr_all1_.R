# Refine distances using random walks
...
foreach(i=1:length(distmfile)) %dopar% {
  tryCatch({
    ...
    for (diagi in 1:nrow(d1)) { d1[diagi,diagi] = 0 }
    
    save(d1,file=paste0(gsub(".Rdata","",distmfile[i]),"_pr_keep<",rwt,".Rdata"))
    }
