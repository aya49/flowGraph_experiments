...
dis = c("manhattan", "euclidean", "rbf","poly","vanilla","tanh","laplace","bessel","anova","spline")#,"euclidean", "canberra")#, "mahalanobis") #
...
disinkernel = c("rbf","poly","vanilla","tanh","laplace","bessel","anova","spline")
disnoneg = c("canberra")
        ...
        for (i in loop.ind) {
          ...
          if (is.null(dim(m))) { #give each layer same weight, m is a list
            if ("none"%in%normalize) { #none
              ...
              if (sum(disinkernel%in%dis[i])>0) dname0 = paste0(dname, "none_dist.Rdata", sep="")
              if (overwrite | !file.exists(dname0)) {
                a = Reduce('cbind',m)
                if (sum(disindist%in%dis[i])>0) {
                  d = dist(a, method=dis[i])
                } else if (sum(disinkernel%in%dis[i])>0) {
                  simmatrix = as.matrix(kernelMatrix(rbfdot(sigma=1),a))
                  save(simmatrix, file=gsub("dist","simmatrix",dname0))
                  d = get_graphd(simmatrix)
                } else {
            ...
            if (("cellpop"%in%normalize | "layer"%in%normalize) & !sum(disinkernel%in%dis[i])>0) { #cell pop
            ...
            if ("cellpop"%in%normalize) {
              ...
              if (sum(disinkernel%in%dis[i])>0) dname1 = paste0(dname, "cellpop_dist.Rdata", sep="")
              if (overwrite | !file.exists(dname1)) {
                if (sum(disindist%in%dis[i])>0) {
                  d = dist(m, method=dis[i])
                } else if (sum(disinkernel%in%dis[i])>0) {
                  simmatrix = as.matrix(kernelMatrix(rbfdot(sigma=1),m))
                  save(simmatrix, file=gsub("dist","simmatrix",dname0))
                  d = get_graphd(simmatrix)
                } else {
            ...
            if ("layer"%in%normalize & !sum(disinkernel%in%dis[i])>0) { #cell pop
              dname2 = paste0(dname, "layer.Rdata", sep="")
