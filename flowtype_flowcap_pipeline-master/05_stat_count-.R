...
#Options for script
interestedCols = c("fileName","aml") #first variable is unique for all files, second variable is one stats enumerated for.
...
#rchy stats
rchyfolders = list.dirs(rchy_dir,full.names=T,recursive=F)
...
result = foreach (rfi = 1:length(rchyfolders)) %dopar% {
  ...
  for (gi in 1:length(uniqueVals)) {
    figi = sampleMeta$fileName[sampleMeta[,interestedCols[2]]==uniqueVals[gi]] #files belonging to gi
    addending = F
    if (!grepl(".fcs$",figi[1]) & grepl(".fcs$",fileNames0[[rfi]][1])) { figi = paste0(figi,".fcs",sep=""); addending = T }
    fnInG = fileNames0[[rfi]][fileNames0[[rfi]] %in% figi] #files belonging to gi in folder rfi
    if (length(fnInG)>0) {
      nodes = edges = NA
      for (fnii in 1:length(fnInG)) {
        fni = fnInG[fnii]
        if (addending) fni = gsub(".fcs","",fni)
  ...
  rchyNodesCommon = cbind(sampleMeta[,interestedCols[2]][match(fileNames0[[rfi]],sampleMeta$fileName)],fileNames0[[rfi]],rchyNodesCommon)
  rchyEdgesCommon = cbind(sampleMeta[,interestedCols[2]][match(fileNames0[[rfi]],sampleMeta$fileName)],fileNames0[[rfi]],rchyEdgesCommon)
  rchyNodesDiff = cbind(sampleMeta[,interestedCols[2]][match(fileNames0[[rfi]],sampleMeta$fileName)],fileNames0[[rfi]],rchyNodesDiff)
  rchyEdgesDiff = cbind(sampleMeta[,interestedCols[2]][match(fileNames0[[rfi]],sampleMeta$fileName)],fileNames0[[rfi]],rchyEdgesDiff)
...
inFile = cbind(sampleMeta[,c("fileName","gene")],nLayers,nSig)
colnames(inFile) = c("fileName","gene","nlayers","nSigNodes")
...
sigFileNot = cbind(sampleMeta[,c("fileName","gene")],sigFileNot)
...
sigFile = cbind(sampleMeta[,c("fileName","gene")],sigFile)

colnames(sigFile) = colnames(sigFileNot) = c("fileName","gene",rownames(m0))
...
result = foreach (uvi = 1:length(uniqueVals)) %dopar% {
  inCommonDir = inCommon = NULL
  uv = uniqueVals[uvi]
  muv = m0[sampleMeta[,interestedCols[2]]==uv,]
...
inComm = cbind(uniqueVals,table(sampleMeta[,interestedCols[2]]),inCommonDir,inCommon,incommonnNOT)
