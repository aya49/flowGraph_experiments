...
for (ci in 1:length(paste0(panelL,centreL))) {
  ...
  for (mcp in matrix_type) {
    ...
    matrix_type. = append(paste0(matrix_dir[ci],matrix_weight,".Rdata"), paste0(matrix_dir[ci],mcp,".Rdata"))
    ...
    for (countThres in cellCountThres) {cat("\ncountThres: ",countThres," >",sep="")
      ...
      for (k in k0) { cat(" level",k," ",sep="")
        ...
        dname = paste(dist_dir[ci], "/", mcp, "_manhattan_layer-", str_pad(k, 2, pad = "0"), "_countThres-", countThres, "_normalize-cellpop", sep = "" )
