...
cltypes = c("distmatrix")#,"knn","kmed","lv","spec1","spec","hc","dc1","dc")#,"spec","dc") #"rw1" (produces one cluster...) #cluster/classify/distmatrix you want to calculate score for
...
if (!is.element("densitycut",installed.packages()[,1])) devtools::install_bitbucket("jerry00/densitycut_dev")
library("densitycut")
libr("vegan")
libr("Rtsne")
libr("fpc")
...
distmfile1 = list.files(dist_clusterf_dir[ci], recursive=F, full.names=T)
