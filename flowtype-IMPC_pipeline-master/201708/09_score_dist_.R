...
dof = T #do scoring
doClusterStats = F # time consuming for IMPC
doplot = T #do plotting
overwritecl = F #redo and overwrite all clusterings
overwritef = F #redo and overwrite all scores
overwriteplot = F #redo and overwrite all plots
...
avgallcol = "" #when averaging distances, match based on "specimen" #doesn't matter if not split
...
cltypes = c("distmatrix","hc")#"knn","kmed","lv","spec1","spec","hc","dc1","dc")#,"spec","dc") #"rw1" (produces one cluster...)
...
interested = c("gene","gender") #sampleMeta columns to plot
splitby = c("none","none") #match with above
...
for (ci in 1:length(paste0(panelL,centreL))) {
  ...
  mcount = get(load(paste0(matrix_dir[ci], matrix_count,".Rdata")))
  
  #add 'label' col to sampleMeta0
  sampleMeta0 = get(load(sampleMeta_dir[ci]))
  # sampleMetaTrain = read.csv(sampleMetaTrain_dir[ci])
  ...
  #make training labels; sample less than half from each type of file from interested attribute with most unique variables
  uv = sapply(interested, function(x) length(unique(sampleMeta0[,x])))
  interesteduv = interested[which.max(uv)]
  labeli = sapply(unique(sampleMeta0[,interesteduv]), function(x) {
    a = which(sampleMeta0[,interesteduv]==x)
    return(sample(a,floor(length(a)/3)))
  })
  label = rep(0,nrow(sampleMeta0))
  label[unlist(labeli)] = NA
  sampleMeta0 = cbind(sampleMeta0,label)
  
  #list out dist files
  distmfile = append(list.files(dist_dir[ci], recursive=F, full.names=F, pattern=".Rdata"),list.files(dist_clustercl_dir[ci], recursive=F, full.names=F, pattern=".Rdata")) 
  ...
  distmfile = unique(distmfile)
  distmfile = distmfile[grepl("CountAdj",distmfile)]
  distmfile = distmfile[grepl("layer06",distmfile)]
  distmfile = distmfile[grepl("manhattan",distmfile)]
  distmfile = distmfile[!grepl("weighted",distmfile)]
  ...
  #   !distmfilenames%in%list.files(dist_clusterf_dir[ci], recursive=F, full.names=F, pattern=".Rdata")]
  # distmfile = distmfile[!distmfile%in%list.files(dist_clusterf_dir[ci], recursive=F, full.names=F, pattern=".Rdata")]
  ...
  loop.ind = c(1:length(distMeta$path))#[!distMeta$path%in%list.files(dist_clusterf_dir[ci],full.names=F)]
  errors = foreach(i=loop.ind, .combine="c") %dopar% {
    ...
    tryCatch({
      ...
      if (i2!=2) distpath = paste0(dist_dir[ci],"/",distMeta$path[i])
      ...
      if (i2==3) { 
        ...
        dtsnefile = paste0(plot_dist_source_dir[ci],"/tsne_",fileNames(distpath))
      ...
      #create score list
      fm = list()
      if (file.exists(paste0(dist_clusterf_dir[ci],"/",fileNames(distpath)))) fm = get(load(paste0(dist_clusterf_dir[ci],"/",fileNames(distpath))))
      
      #create classification/clustering list
      cm0 = list()
      if (file.exists(paste0(dist_clustercl_dir[ci],"/",fileNames(distpath)))) { cm0 = get(load(paste0(dist_clustercl_dir[ci],"/",fileNames(distpath))))
        ...
        for (dindname in names(sm)) {
            ...
            for (cltype in cltypes) {
              ...
              if (cltype=="lv") { #input is a similarity matrix
                if (grepl("dist.Rdata",distpath)) {
                  sim0 = get(load(paste0(dist_dir[ci],"/",gsub("dist.Rdata","simmatrix.Rdata",distpath))))
              ...
              for (pari in 1:length(pars)) {
                ...
                if (!cltype%in%cltypesclass & !cltype%in%cltypesorigm) {
                  ...
                  if (!"silmed"%in%names(fm[[colnam]][[dindname]][[cltype]][[par]]) | overwritef) {
                    ...
                    } else { sil = median(silhouette(cl,as.dist(d[[dindname]]))[,3]) }
                    fm[[colnam]][[dindname]][[cltype]][[par]]["silmed"] = sil
                  }
                  
                  # TAKES A LONG TIME with IMPC
                  if (doClusterStats) {
          ...
          try({ #never stop the loop because of plotting errors!
            if (doplot & i2!=2 & (length(cm0[[colnam]][[dindname]])>0 & length(fm[[colnam]][[dindname]])>0) & ((!dindname=="all" & length(sm)>1) | (dindname=="all" & length(sm)==1))) {
              ...
              #prepare png
              pngname = paste0(plot_dist_eval_dir[ci], "/tsne_", gsub(".Rdata","",fileNames(distpath)),"_class_",interested[col],"-",splitby[col],"-",dindname,"__",paste(cltypes0,collapse="-"),".png")
              if (!file.exists(pngname) | overwriteplot) {
                ...
                for (cltype in names(cm0[[colnam]][[dindname]])) {
                  ...
                  # pngname = paste0(plot_dist_eval_dir[ci], "/tsne_", gsub(".Rdata","",fileNames(distmfile[i])),"_class_",interested[col],"-",splitby[col],names(d)[dind],"_",cltype,".png")
      ...
      if (dof) save(fm,file=paste0(dist_clusterf_dir[ci],"/",fileNames(distpath)))
      if (docl) save(cm0,file=paste0(dist_clustercl_dir[ci],"/",fileNames(distpath)))
...
distMeta$path[!distMeta$path%in%(list.files(dist_clusterf_dir[ci],full.names=F))]
