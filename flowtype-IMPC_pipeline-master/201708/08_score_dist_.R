...
for (ci in 1:length(paste0(panelL,centreL))) {
  ...
  matrix_type = list.files(path=result_dir[ci],pattern=glob2rx("matrix*.Rdata"))
  ...
  errorsb = foreach (mcp = loop.ind) %dopar% {
    tryCatch({
      if (!file.exists(paste0(matrix_dir[ci], mcp,".Rdata"))) {cat("doesn't exist"); return(NULL)}
      ...
      for (countThres in cellCountThres) { cat("\ncountThres: ",countThres," >",sep="")
        ...
        for (k in ifelse(grepl("TRIM",mcp), k0[!k0==1], k0)) { cat(" level",k," ",sep="")
          ...
          dnamee = paste(dist_dir[ci], "/", mcp, "_rbf_layer-", str_pad(k, 2, pad = "0"), "_countThres-", countThres, "_normalize-",norm,".Rdata", sep="")
          dnameo = paste(dist_clustercl_dir[ci], "/", mcp, "_other_layer-", str_pad(k, 2, pad = "0"), "_countThres-", countThres, "_normalize-",norm,".Rdata", sep="")
...
aa = distMetafun(list.files(dist_clustercl_dir[ci],pattern="other"),dis=c(dis,"other"))
