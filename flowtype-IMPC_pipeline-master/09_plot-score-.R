...
#Input
phenoMeta_dir = paste(result_dir, "/", panelL, "/", centreL, "/phenoMeta.Rdata", sep="") #metadata for cell populations (phenotype)
sampleMeta_dir = paste(result_dir, "/", panelL, "/", centreL, "/sampleMeta.Rdata", sep="") #metadata for FCM files
# sampleMetaTrain_dir = paste0("attachments/AMLTraining.csv") #which FCM files are testing/training files
matrix_dir = paste(result_dir, "/", panelL, "/", centreL, "/matrix", sep="") #feature matrices
...
dist_comp_dir = paste(result_dir, "/", panelL, "/", centreL, "/dist_compare", sep="")
...
dist_comp_dist_dir = paste(result_dir, "/", panelL, "/", centreL, "/dist_compare_dist", sep="")
...
cltypes0 = c("distmatrix","hc") #, "spec1","knn","kmed","hc","lv" #for plot and table making
...
matrix_type_features = c("CountAdj","Prop","CountAdjPEER","LogFold","Pval","Child_entropy","Parent_entropy","Child_prop","Child_pnratio","Parent_contrib","Parent_effort","Freqp0.5")
...
plotcols = c("type","layer","dist") #split columns to plot
...
#Prepare data
distmfile1 = list.files(dist_clusterf_dir[ci], recursive=F, full.names=T, pattern=".Rdata$")

#filter files
distmfile1 = distmfile1[grepl("CountAdj",distmfile1) & grepl("layer06",distmfile1) & grepl("manhattan",distmfile1)]
...
distmfile11 = gsub("CountAdjPEER","PEER",distmfile1) # temporary fix so that distMetaFun works
distMeta1 = distMetafun(distmfile11,c(dis,"other"),features=matrix_type_features)
distMeta1[,c("path","type","feature","featend")] = sapply(c("path","type","feature","featend"), function(x) gsub("PEER","CountAdjPEER",distMeta1[,x])) # temporary fix so that distMetaFun works
# distMeta1$size = fs

distmfile2 = list.files(dist_dir[ci], recursive=F, full.names=T, pattern=".Rdata$")

#filter files
distmfile2 = distmfile2[(grepl("CountAdj",distmfile2) | grepl("PEER",distmfile2)) & grepl("layer06",distmfile2) & grepl("manhattan",distmfile2)]

distMeta2 = distMetafun(distmfile2,dis=c(dis,"other"),features=matrix_type_features) #for comparing dist matrices with
...
save(score1,file=paste0(dist_score_all_dir[ci],".Rdata"))
#write.csv(score1,file=paste0(dist_score_all_dir[ci],".csv"))
...
score1 = get(load(paste0(dist_score_all_dir[ci],".Rdata")))
scorecol = 18 # scorecol = which(colnames(score1)=="clusters")+1 #column where score starts, can also just look at the score only matrix...
...
maxlayeronly = T #for non-layer parameters, use only maximum number of layers
  ...
  tryCatch({
    ...
    for (score in scorecol:ncol(score3)) {
      ...
      for (cltype in cltypes0) {
        ...
        scorename0 = displayscore[originalscore==scorename]
        ...
        for (parr in pars) {
          ...
          ## plot
          # pngname = paste0(plot_compare_dir[ci],"_thesis/", scorename,"__",cltype,"__",colnam,"__par-",parr,".png")
          pngname = paste0(plot_compare_dir[ci],"/", scorename,"__",cltype,"__",colnam,"__par-",parr,".png")
          png(filename=pngname, height=height*length(plotcols), width=width)#*(length(unique(score4$dindname))))
          par(mfcol=c(length(plotcols),length(pars)), mar=c(25,6,3,15), xpd=T)
          ...
          for (dindname in sort(unique(score4$dindname))) {
            score60 = score5[score5$dindname==dindname,]
            if (dindname=="avg") score60 = score5[!score5$dindname%in%c("all","avg"),]
            main1 = paste0(colnam,"-",dindname)

            #for each plot
            for (splitcolsi in 1:length(plotcols)) {
              splitcols = plotcols[splitcolsi]
              
              score6 = score60
              filterind = rep(T,nrow(score6))
              if (maxlayeronly & splitcols!="layer") filterind = score6[,"layer"] == max(score6[,"layer"])
              score6 = score6[filterind,]
              
              #y label
              splitcols0 = plotcols0[splitcolsi]
              ...
              y_par = NULL; if (length(unique(score6$par))>1) y_par = score6$par
              ...
              if (!is.null(y_par)) text(x_split, y_score, labels=y_par, cex= 1.2, pos=3)
...
## Plot distance between distances----------------------------------------------------------

if (doDistPlot) {
  start2 = Sys.time()
  
  if (overwritedist | !file.exists(paste0(dist_comp_dir[ci],".Rdata"))) {
    ...
    save(d0,file=paste0(dist_comp_dir[ci],".Rdata"))
  } else {
    d0 = get(load(paste0(dist_comp_dir[ci],".Rdata")))
  }
  ...
  cat(", distances ")
  if (overwritedist | !file.exists(paste0(dist_comp_dist_dir[ci],".Rdata"))) {
    did = dist(d)
    did1 = dist(d1)
  } else {
    did0 = get(load(paste0(dist_comp_dist_dir[ci],".Rdata")))
    did = did0$did
    did1 = did0$did1
  }
  TimeOutput(start)
  ...
  if (doRtsne) {
    ...
    png(filename=paste0(plot_compare_dir[ci],"/dist__",paste(plotscores,collapse="-"),".png"), width=width*length(plotscores), height=height)
  ...
  if(doPCA) {
    ...
    for (plotscore in plotscores) {
      ...
      for (ii in c(1:2)) {
        ...
        png(filename=paste0(plot_compare_dir[ci],"/distcomparePCA_",plotscore,"_",ii,".png"), width=600, height=600*(1+plotpc))
