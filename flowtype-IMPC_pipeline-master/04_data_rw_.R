...
#Output
rw_dir = paste(result_dir,  "/", panelL, "/", centreL, "/rw", sep=""); for (i in 1:length(rw_dir)) { suppressWarnings(dir.create (rw_dir[i])) }
...
good_samples = c(3)
controlL = c("+_+|+_Y","+_+|+_Y","WildType","WildType","WildType") #control value in target_col column for each centre
date_col = "date"
target_col = "gene"
...
dis = c("rw")
min_steps = 100
# tube = 4 #which panel to use (flowCAP-II only); any of tubes 1-7

#data paths
#matrix_type = list.files(path=result_dir,pattern=glob2rx("matrix*.Rdata"))
matrix_type = list(c("Pval_CountAdj","",F,T)) #node, edge, if edge is blank, use node values on edges (of child node); edge weight should be reversed or not; should edge weights be absolute
...
for (ci in 1:length(paste0(panelL,centreL))) {
  ...
  for (mcp in matrix_type) {
    tryCatch({
      ...
      if (file.exists(paste0(matrix_dir[ci], mcp[1],".Rdata"))) { m0 = get(load(paste0(matrix_dir[ci], mcp[1],".Rdata")))
      } else if (file.exists(paste0(matrix_dir[ci], mcp[1],".csv"))) { m0 = read.csv(paste0(matrix_dir[ci], mcp[1],".csv", header=T))
      } else { next } 
      
      ## upload and prep edge matrix
      if (mcp[2]!="") {
        if (file.exists(paste0(matrix_dir[ci], mcp[2],".Rdata"))) { e0 = get(load(paste0(matrix_dir[ci], mcp[2],".Rdata")))
        } else { next }
      }
      ...
      for (countThres in cellCountThres) { cat("\ncountThres: ",countThres," >",sep="")
        ...
        mm = trimMatrix(m0, TRIM=grepl("TRIM",mcp[1]), mc=mc,
                        sampleMeta=sampleMeta, sampleMeta_to_m1_col="fileName", target_col=target_col, control=str_split(controlL[ci],"[|]")[[1]], order_cols=NULL, 
        ...
        dname = paste(rw_dir[ci], "/", mcp[1], ifelse(mcp[2]!="","_edge-",""), mcp[2], "_", dis, "_layer-", str_pad(k, 2, pad = "0"), "_countThres-", countThres, "_goodSample-",good_sample, sep = "")
