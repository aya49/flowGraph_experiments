...
control = controlL[ci] #control value in target_col column
    ...
    for (k in layers) {
      ...
      for (tube in names(split_ind)) {
        ...
        for (cmethod in cmethods) {
          ...
          if (colhaslayer) {
            dname0 = paste0("/",feat_type, "_dist-NA", sep = "")
          } else {
            dname0 = paste0("/",feat_type, "_layer-", str_pad(k, 2, pad = "0"), "_countThres-", countThres, "_dist-NADIST")
          }
