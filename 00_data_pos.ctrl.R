    ...
    llply(ii, function(i) {
      ...
      if (i>(nsample*nctrl) & grepl("pos",ds)) {
        ...
        if (ds=="pos1") { # A+ > .75; A- > .25
          ...
          tripleind = which(ap)
          tm = sum(double)
          f@exprs = f@exprs[-sample(tripleind,tm),]
        } 
