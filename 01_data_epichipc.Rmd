...
indices <- c(25,45,27,47,26,46,28,48)
...
i=indices[4]    
# print(fg@summary_desc$node[i,1:2])
headp <- sort(fg@summary$node[[i]]$values)[fg_get_summary(fg, index=55)$values==0]
# print(headp)
...
i=indices[8]    
# print(fg@summary_desc$node[i,1:2])
headp <- sort(fg@summary$node[[i]]$values[fg_get_summary(fg, index=56)$values==0])
# print(headp)
for (j in names(headp)[1:8])
    plot(fg_plot_box(fg, index=i, cpop=j, paired=FALSE, dotplot=FALSE))
```
