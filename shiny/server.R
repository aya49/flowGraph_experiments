...
data_dir <- c(data="/mnt/f/FCS data/Tobias Kollmann")
start_dir <<- c(data="/mnt/f")
...
server <- function(input, output, session) {
    ...
    output$table_summary = DT::renderDataTable({
        ...
        # input$go # re-run when button is clicked

        # withProgress(message='loading data', value=0, {
        #     incProgress(.3, detail="uploading flowGraph object")

        # fg_dir <<- paste0(shinyFiles::parseDirPath(start_dir, input$fg_dir), collapse="")
        # fg <<- fg_load(fg_dir)
        ...
        # incProgress(.9, detail="preparing summary statistics table")
    ...
    output$plot_hierarchy <- renderGirafe({
        req(input$table_summary_rows_selected)
        ...
        withProgress(message='loading', value=0, {
            ...
            # plot cell hierarchy
            gr <- fg_plot(fg, index=index, p_thres=input$p_thres, show_bgedges=FALSE, label_max=input$max_nodes, node_labels=c("cells_ul_blood", "expect_cells_ul_blood"))
    ...
    output$plot_box <- renderGirafe({
        req(input$plot_hierarchy_selected,
            input$paired, input$dotplot, input$outlier)
        ...
        gg_box <- fg_plot_box(fg, node_edge=cpop, paired=paired, outlier=outlier, dotplot=dotplot, shiny_plot=TRUE)
    ...
    output$plot_dens <- renderPlot({
        req(input$plot_hierarchy_selected, input$plot_box_selected,
            input$ft_folder, input$fcs_folder)
        cpop <- input$plot_hierarchy_selected
        label <- input$plot_box_selected
        ft_dir_ <- input$ft_folder
        fcs_dir_ <- input$fcs_folder

        withProgress(message='loading', value=0, {
            incProgress(.2, detail="loading median samples")

            ft_dir <<- paste0(shinyFiles::parseDirPath(start_dir, ft_dir_), collapse="")
            fcs_dir <<- paste0(shinyFiles::parseDirPath(start_dir, fcs_dir_), collapse="")
