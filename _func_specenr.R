...
loop_ind_f <- function(x,n) {
  ...
  return(split(x, ceiling(seq_along(x)/ceiling(base::length(x)/n))))
}
...
gsub_markers <- function(fg, markers_new, markers_old=NULL) {
  require(plyr)
  ...
  for (mi in 1:base::length(markers_old)) {
    fg@graph$v$phenotype =
      gsub(markers_old[mi], markers_new[mi], fg@graph$v$phenotype)
    fg@graph$e$from =
      gsub(markers_old[mi], markers_new[mi], fg@graph$e$from)
    fg@graph$e$to =
      gsub(markers_old[mi], markers_new[mi], fg@graph$e$to)

    fg@edge_list$parent = llply(fg@edge_list$parent, function(x)
      gsub(markers_old[mi], markers_new[mi], x))
    names(fg@edge_list$parent) = gsub(markers_old[mi], markers_new[mi], names(fg@edge_list$parent))
    fg@edge_list$child = llply(fg@edge_list$child, function(x)
      llply(x, function(y) {
        gsub(markers_old[mi], markers_new[mi], y)
      }))
    names(fg@edge_list$child) = gsub(markers_old[mi], markers_new[mi], names(fg@edge_list$child))

  }
  fg@feat$node = llply(fg@feat$node, function(x) {
    colnames(x) = fg@graph$v$phenotype
    x
  })
  if (base::length(fg@feat$edge)>0) {
    ecn = paste0(fg@graph$e$from, "_", fg@graph$e$to)
    fg@feat$edge = llply(fg@feat$edge, function(x) {
      colnames(x) = ecn
      x
    })
  }
...
# replace ids
gsub_ids <- function(fg, ids_new, ids_old=NULL) {
  require(plyr)

  if (is.null(ids_old)) {
    if (base::length(ids_new)!=nrow(fg@meta)) {
      cat("incorrect number of ids\n")
      return(fg)
    }
    ids_old = fg@meta$id
  } else {
    if (base::length(ids_old)!=base::length(ids_new)) {
      cat("incorrect number of ids\n")
      return(fg)
    }
  }

  ids_ind = match(ids_old, fg@meta$id)
...
extract_samples <- function(fg, sample_ids) {
  require(plyr)
  ...
  id_inds = match(sample_ids,fg@meta$id)

  fg@meta = fg@meta[id_inds,]
  fg@feat$node = llply(fg@feat$node, function(x) x[id_inds,,drop=F])
  if (base::length(fg@feat$edge)>0) {
    fg@feat$edge = llply(fg@feat$edge, function(x) x[id_inds,,drop=F])
  }
  return(fg)
}
...
extract_phenotypes <- function(fg, phenotypes) {
  require(plyr)
  ...
  id_inds = match(phenotypes,fg@graph$v$phenotype)
  id_inds_ = fg@graph$e$from%in%phenotypes & fg@graph$e$to%in%phenotypes

  fg@graph$v = fg@graph$v[id_inds,,drop=F]
  fg@graph$e = fg@graph$e[id_inds_,,drop=F]
  fg@graph = set_layout_graph(fg@graph, as.function(fg@plot_layout))

  fg@feat$node = llply(fg@feat$node, function(x) x[,id_inds,drop=F])
  if (base::length(fg@feat$edge)>0) {
    fg@feat$edge = llply(fg@feat$edge, function(x) x[,id_inds_,drop=F])
  }

  fg@edge_list$child =
    fg@edge_list$child[phenotypes[phenotypes%in%names(fg@edge_list$child)]]
  fg@edge_list$child = llply(fg@edge_list$child, function(x)
    llply(x, function(y) {
      a = y[y%in%phenotypes]
  ...
  fg@edge_list$child = plyr::compact(fg@edge_list$child)
  fg@edge_list$parent =
    fg@edge_list$parent[phenotypes[phenotypes%in%names(fg@edge_list$parent)]]
  fg@edge_list$parent = llply(fg@edge_list$parent, function(x) {
    a = x[x%in%phenotypes]
    if (base::length(a)==0) return(NULL)
    a
  })
  fg@edge_list$parent = plyr::compact(fg@edge_list$parent)
...
merge_samples <- function(obj1, obj2) {
  ...
  if (base::length(fg@feat$edge)>0) {
    ...
    fg@feat$edge = llply(efs, function(xi) {
      a = obj1@feat$edge[[xi]]
      b = obj2@feat$edge[[xi]]
      abcol = intersect(colnames(a),colnames(b))
      ab = rbind(a[,match(abcol,colnames(a)),drop=F],
...
tmm <- function(x, x0=NULL, lib.size, refColumn, cutoff=Inf, pngnames=NULL, mains=NULL, no_cores=1, samplesOnCol=F,
  ...
  ff = foreach(i=1:ncol(x), .combine = list, .maxcombine = ncol(x), .multicombine = T) %dopar% {
    ...
    if (plotimg) {
      ...
      plot(d, main=paste0("density of TMM ratios of counts between current and reference sample ",colnames(x)[refColumn], "\nblue=density peak, red=TMM value", "\nfinal TMM value in ", ifelse(abs(fi-p1)>cutoff, "blue","red"))); abline(v=fi, col="red"); abline(v=p1, col="blue");
  ...
  fdiff = rep(NA,base::length(ff)) # diff between density peak and value (note: logged)
  for (i in 1:base::length(ff)) {
    f[i] = ff[[i]]$f
...
getPhen <- function(phen, phenocode=NULL) {
  ...
  if (is.null(phenocode)) {
    pm$phenocode = laply(phen, function(x){
      if (x=="") return(paste0(rep(0,base::length(markers)), collapse=""))
      b = str_split(x,"[+-]+")[[1]]
      b = b[-base::length(b)]
      bo = match(markers,b)
...
getPhenCP <- function(meta_cell=NULL, cp=NULL, no_cores=1) {
  ...
  allcol = llply(1:base::length(allcolu), function(ci) {
  ...
  names(pchild) = ""
  pparen = llply(1:base::length(pchild[[1]]), function(x) "")
  ...
  edf = do.call(rbind, llply(1:base::length(pchild), function(x)
...
# change layout
set_layout <- function(fg, layout_fun=NULL) {
  fg@plot_layout = as.character(substitute(layout_fun))
...
flowgraph <- function(input_, meta=NULL, no_cores=1,
  ...
  if (class(input_)=="integer" | class(input_)=="matrix") {
    ...
    if (class(input_)=="Phenotypes") {
      ...
      names(ftl) = laply(str_split(input_,"/"), function(x) x[base::length(x)])
    } else {
      badinput = T
    }
    if (badinput) {
  ...
  pchild_ = llply(pchild_, function(x) {
    ...
    if (all(sapply(a, base::length)==0)) return(NULL)
    return(a)
  })
...
flowgraph_specenr <- function(fg, no_cores=1, overwrite=F) {
  ...
  expe1 = matrix(.5,nrow=nrow(mp), ncol=base::length(cells1), dimnames=list(rownames(mp),cells1))
  expe1[,1] = 1
  ...
  expecp = do.call(cbind, llply(loop_ind, function(ii) {
    do.call(cbind, llply(ii, function(ic) {
      ...
      pedges = do.call(cbind,llply(1:base::length(pnames), function(gi) {
...
flowgraph_mean_class <- function(fg, class, no_cores=1, node_features=NULL, edge_features=NULL) {
  ...
  if (base::length(unique(fg@meta[,class]))<2) return(fg)
...
flowgraph_p <- function(
  ...
  if (!is.null(control)) {
    ...
    class_list = llply(2:base::length(classes), function(i)
    ...
    for (feature in features) {
      ...
      for (classl in class_list) {
        ...
        if (!diminish) {
          ...
          if (ftype=="node") p_thress = rep(p_rate, base::length(layers)-1)
          if (ftype=="edge") p_thress = rep(p_rate, base::length(layers))
          p_thress[1] = p_thres
        ...
        fg@feat_summary[[ftype]][[feature]][[test_name]][[class]][[idsname]][["m1"]] = m1_ = colMeans(as.matrix(m1))
        fg@feat_summary[[ftype]][[feature]][[test_name]][[class]][[idsname]][["m2"]] = m2_ = colMeans(as.matrix(m2))
...
flowgraph_cumsum <- function(fg, no_cores) {
  ...
  meta_cell_gridTF = apply(meta_cell_grid,2, function(x) {
    xmax = max(x)
    if (xmax==2) return(rep(F,base::length(x)))
    x>1 & x<xmax
  })
...
setMethod(
  "show", "flowgraph",
  function(fg)
    cat("flowgraph object with ", base::length(fg@feat$node)," cell population and ", base::length(fg@feat$edge), " edge feature(s)",
        "\n- markers: ", paste0(fg@markers, collapse=", "),
        "\n- contains: ", nrow(fg@graph$v), " cell populations and ", nrow(fg@graph$e), " edges\n", sep="")
)
...
setMethod(
  ...
  function(fg) {
    ...
    result1 = ldply(names(fg@feat$node), function(x)
      summary_table(fg@feat$node[[x]],x))
    result1 = data.frame(type=rep("node",nrow(result1)),result1)

    result2 = NULL
    if (base::length(fg@feat$edge)>0) {
      result2 = ldply(names(fg@feat$edge), function(x)
