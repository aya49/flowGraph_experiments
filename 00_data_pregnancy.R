## adapted from albina's code to gate immune clock pregnancy data set
...
source("source/pregnancy/preProcessingFunc.R") ## Function for the pre-Processing of the datasets
source("source/pregnancy/globalFrameFunc.R") ## Function for creating a Global Frame
libr(c("plyr",
       "foreach", "doMC"))
...
suppressWarnings(dir.create ("/mnt/f/Brinkman group/current/Alice/temp_pregnancy"))

## Paths to the FCS files, metadata spreadsheets, and output folder in the Bioinformatics drive
if(dataInfo == "Training"){
  inPath <- "/mnt/f/FCS data/Immune_Clock_Human_Pregnancy/FR-FCM-ZY3Q"
  inputPath <- list(inPath)
  suppressWarnings(dir.create ("/mnt/f/Brinkman group/current/Alice/temp_pregnancy/FR-FCM-ZY3Q/"))
  outputPath <- "/mnt/f/Brinkman group/current/Alice/temp_pregnancy/FR-FCM-ZY3Q/"
  
}else if(dataInfo == "Validation"){
  inPath <- "/mnt/f/FCS data/Immune_Clock_Human_Pregnancy/FR-FCM-ZY3R"
  inputPath <- list(inPath)
  suppressWarnings(dir.create ("/mnt/f/Brinkman group/current/Alice/temp_pregnancy/Results/FR-FCM-ZY3R/"))
  outputPath <- "/mnt/f/Brinkman group/current/Alice/temp_pregnancy/FR-FCM-ZY3R/"
}
