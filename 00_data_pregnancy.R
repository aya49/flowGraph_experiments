...
## options
widthheight = 300 # plot width/height
overwrite = F
...
if (overwrite | !file.exists(gates_dir)) {
  gates = llply(fslist, function(leukocytes) { 
    ...
    # tryCatch({
      ...
      all.filters[["cd4.t.memory"]] = Tregs.Memory.flowD@filter
...
btf = llply(gthres, function(x) return(quadGate("CD3"=x["cd3"], "CD19"=x["cd19"])))
...
nkf = llply(gthres, function(x) return(quadGate("CD14"=x["cd14.high"], "CD7"=x["cd7"])))
...
mcf = llply(gthres, function(x) return(quadGate("CD14"=x["cd14.low"], "CD16"=x["cd16.LinNeg"])))
...
cd45f = llply(gthres, function(x) return(quadGate("CD45RA"=x["cd45RA.high.cd4"])))
nodeIDs <- flowWorkspace::add(gs, cd45f, parent="cd4tcell")
# cd4.T.Naive = CD45+
# cd4.T.Memory = CD45-
recompute(gs)

tbet = llply(gthres, function(x) return(quadGate("Tbet"=x["tbet.cd4t"])))
nodeIDs <- flowWorkspace::add(gs, tbet, parent="CD45+")
# cd4.Th1.Naive = Tbet+
nodeIDs <- flowWorkspace::add(gs, tbet, parent="CD45-")
# cd4.Th1.Memory = Tbet+
recompute(gs)
...
nodeIDs <- flowWorkspace::add(gs, tnf, parent="CD45+")
recompute(gs)

tmf = llply(filters, function(x) return(polygonGate(filterId="tregs.memory", .gate=x$cd4.t.memory)))
nodeIDs <- flowWorkspace::add(gs, tmf, parent="CD45-")
recompute(gs)
...
cd8tf = llply(gthres, function(x) return(quadGate("CD45RA"=x["cd45ra.cd8"])))
nodeIDs <- flowWorkspace::add(gs, cd8tf, parent="cd8tcell")
# cd8.T.Naive= CD45RA+
# cd8.T.Memory= CD45RA+
recompute(gs)

tbetf = llply(gthres, function(x) return(quadGate("Tbet"=x["tbet.cd8t"])))
nodeIDs <- flowWorkspace::add(gs, tbetf, parent="CD45RA+")
# cd25.cd8.T.Naive = Tbet+
nodeIDs <- flowWorkspace::add(gs, tbetf, parent="CD45RA-")
# cd25.cd8.T.Memory = Tbet+
recompute(gs)
